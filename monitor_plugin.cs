...
class Program {
  ...
  static float get_sensor_by_path(string section, string sensor_type, string sensor_name) {
    ...
    foreach (var device in section_list) {
      List<dynamic> sensor_list = device[sensor_type].ToObject<List<dynamic>>();
      if (sensor_list.Count != 0) {
        foreach (var sensor in sensor_list) {
  ...
  static float get_sensor(Effect e) {
    ...
    if (e.name == "cpu_temp") {
      ...
      x = get_sensor_by_path("gpus", "temps", "GPU");
      x = Tools.remap(x, 40, 70, 0, 1);
    } else if (e.name == "gpu_load") {
      x = get_sensor_by_path("gpus", "loads", "GPU");
      x = Tools.remap(x, 0, 100, 0, 1);
    }
    return x;
  }
